...
VL53L0X_Error vl53l0x_begin(I2C_HandleTypeDef *i2c, xshut *xshut, int nb_sensors){
	...
	for(int i=0; i<nb_sensors; i++)
	{
		...
		HAL_GPIO_WritePin(cfg[i].pin_xshut.GPIOx, cfg[i].pin_xshut.GPIO_Pin, GPIO_PIN_RESET);
	}
...
VL53L0X_Error vl53l0x_init(){
	...
	for(int j=0; j<nb_tof; j++)
	{
		...
		HAL_GPIO_WritePin(cfg[j].pin_xshut.GPIOx, cfg[j].pin_xshut.GPIO_Pin, GPIO_PIN_SET);
		HAL_Delay(250);
		...
		// Set new i2c address
		if((status = VL53L0X_SetDeviceAddress(cfg[j].dev, 0x52 + (j+1) * 2)) != VL53L0X_ERROR_NONE){
...
VL53L0X_Error _vl53l0x_Device_Initialization(uint8_t tof_id)
{
	...
	//printf("Sensor %d: Data init done...\r\n", tof_id);
	...
	//printf("Sensor %d: Static init done...\r\n", tof_id);
...
VL53L0X_Error _vl53l0x_Calibration_Data_Load(uint8_t tof_id)
{
	...
	if((status = VL53L0X_PerformRefSpadManagement(cfg[tof_id].dev, &count, &isApertureSpads)) != VL53L0X_ERROR_NONE){
		printf("Error VL53L0X_PerformRefSpadManagement() : %d\r\n",status);
		return status;
	}

	uint8_t VhvSettings, PhaseCal;
	if((status = VL53L0X_PerformRefCalibration(cfg[tof_id].dev, &VhvSettings, &PhaseCal)) != VL53L0X_ERROR_NONE){
...
VL53L0X_Error _vl53l0x_System_Settings(uint8_t tof_id)
{
	...
	// Device Mode
	if((status = VL53L0X_SetDeviceMode(cfg[tof_id].dev, VL53L0X_DEVICEMODE_CONTINUOUS_RANGING)) != VL53L0X_ERROR_NONE){
	...
	if((status = VL53L0X_SetGpioConfig(cfg[tof_id].dev, 0, VL53L0X_DEVICEMODE_CONTINUOUS_RANGING,
			VL53L0X_GPIOFUNCTIONALITY_NEW_MEASURE_READY, VL53L0X_INTERRUPTPOLARITY_HIGH)) != VL53L0X_ERROR_NONE){
...
VL53L0X_Error _vl53l0x_Range_Profiles(uint8_t tof_id)
{
	...
	VL53L0X_SetLimitCheckValue(cfg[tof_id].dev, VL53L0X_CHECKENABLE_SIGNAL_RATE_FINAL_RANGE, signalLimit);
	VL53L0X_SetLimitCheckValue(cfg[tof_id].dev, VL53L0X_CHECKENABLE_SIGMA_FINAL_RANGE, sigmaLimit);
	VL53L0X_SetMeasurementTimingBudgetMicroSeconds(cfg[tof_id].dev, timingBudget);
	VL53L0X_SetVcselPulsePeriod(cfg[tof_id].dev, VL53L0X_VCSEL_PERIOD_PRE_RANGE, preRangeVcselPeriod);
	VL53L0X_SetVcselPulsePeriod(cfg[tof_id].dev, VL53L0X_VCSEL_PERIOD_FINAL_RANGE, finalRangeVcselPeriod);
...
uint16_t vl53l0x_PerformRangingMeasurement(uint8_t tof_id){
	...
	VL53L0X_GetRangingMeasurementData(cfg[tof_id].dev, &VL53L0X_RangingMeasurementData);
